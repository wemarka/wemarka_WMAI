...
[Summary of the report and next steps]`,
  ...
  const handleGenerateDocument = async () => {
    ...
    try {
      ...
      // Simulate AI response with enhanced content
      setTimeout(() => {
        // Create an enhanced version of the document with professional formatting
        const enhancedDocument = createEnhancedDocument(
          activeTab,
          documentTitle,
          documentContent,
        );
  ...
  // Helper function to create enhanced document content
  const createEnhancedDocument = (
    type: string,
    title: string,
    baseContent: string,
  ): string => {
    const currentDate = new Date().toLocaleDateString();
    const companyName = "Wemarka Inc.";

    // Add professional formatting based on document type
    switch (type) {
      case "invoice":
        return `INVOICE
...
From: ${companyName}
To: [Client Name]
Invoice #: ${title.replace("Invoice #", "")}
Date: ${currentDate}
Due Date: ${new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toLocaleDateString()}

${baseContent.includes("Description") ? baseContent.split("Description")[1] : baseContent}
...
This document was generated by Wemarka WMAI Business Operating System`;
...
${baseContent.includes("EXECUTIVE SUMMARY") ? baseContent.split("EXECUTIVE SUMMARY")[1].split("\n\n")[1] : "This proposal outlines our approach to meeting your business needs."}
...
[Detailed pricing breakdown and payment schedule]
...
[Actions required to proceed with the proposal]

Thank you for considering ${companyName} for your business needs.
...
Between: ${companyName} ("Provider")
And: [Client Name] ("Client")
Date: ${currentDate}

${
  baseContent.includes("SCOPE OF SERVICES")
    ? baseContent
    : `1. SCOPE OF SERVICES

${companyName} agrees to provide the following services to the Client:
- [Service 1]
- [Service 2]
- [Service 3]
...
${
  baseContent.includes("EXECUTIVE SUMMARY")
    ? baseContent
    : `1. EXECUTIVE SUMMARY
              ...
              <div className="flex justify-end space-x-2">
                <Button
                  ...
                  onClick={() => {
                    ...
                    const template =
                      documentTemplates[
                        tabValue as keyof typeof documentTemplates
                      ];
                    if (template) {
                      setDocumentTitle(template.title);
                      setDocumentContent(template.content);
